...
const int NUM_VAR_RESISTORS = sizeof(varResistorPins) / sizeof(varResistorPins[0]);
Brightness ledBrightness (LDR_PIN, varResistorPins);
unsigned long checkLightLevelTO = 0;  // Yep, work out when to next check it.
...
void _strobeClockLed() {
  digitalWrite(clockLedPin[currLed], LOW);      // Turn the current digit off.
...
void setup() {
  ...
  for (int i = 0; i < NUM_VAR_RESISTORS; i++) {
    pinMode(varResistorPins[i], OUTPUT);
    digitalWrite (varResistorPins[i], LOW);     // Turn the resistor off
  }                                             // Turn on the first transistor which means no resistors 
  digitalWrite(varResistorPins[0], HIGH);       // in the variable resistance chain => Max brightness
